...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "save" && $NomeTitular != "") {
    try {
        if ($id != "") {
            $stmt = $connection->prepare("UPDATE dbo.DadosRescisao SET CPF=?, NomeTitular=?, Observacao=? WHERE id = ?");
            $stmt->bindParam(4, $id);
        } else {
            $stmt = $connection->prepare("INSERT INTO dbo.DadosRescisao (CPF, NomeTitular, Observacao) VALUES (?, ?, ?)");
        }
